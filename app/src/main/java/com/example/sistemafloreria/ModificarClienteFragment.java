...
public class ModificarClienteFragment extends Fragment implements View.OnClickListener {
    EditText id, nombre, edad, telefono, sexo;
    Button Buscar, Limpiar, modifica;

    public ModificarClienteFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.fragment_modificarcliente, container, false);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.botonbuscar_b:
                ...
                break;
            case R.id.modifica_m:
                Toast.makeText(getContext(), "Entre con exito", Toast.LENGTH_LONG).show();
                String clav = id.getText().toString(), updatedName = nombre.getText().toString(), updatedEdad = edad.getText().toString(),updatedtel = telefono.getText().toString(),updatedsexo = sexo.getText().toString();
                try {
                    adminDB bd = new adminDB(getContext(), "ejercicio", null, 1);
                    SQLiteDatabase con = bd.getReadableDatabase();
                    String Query = "UPDATE clientes SET nombre_cli='"+updatedName+"' edad='"+updatedEdad+"' telefono='"+updatedtel+"' sexo='"+updatedsexo+"' where='"+clav+"';";
                    Cursor c = con.rawQuery(Query, null);
                    bd.close();
                    Toast.makeText(getContext(), "Se ha modificado con exito", Toast.LENGTH_LONG).show();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
